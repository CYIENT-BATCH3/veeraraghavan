...
enum modes{CONFIG_MODE = 0X80,NORMAL_MODE_TRANS = 0X08,NORMAL_MODE_RECEIV = 0X0E};
...
void find_fault_data(void);
void cc_led_change(void);
void change_err_stat_nLED(void);
void set_byte_transmission();
void nrcs_data_extract(void);
void find_distance_from_nrcs(void);
# 9 "CAN_MAIN.C" 2
...
char cc_nrcs_stat = 0;
char cc_switch_stat = 0;
char speed_cmd = 0;
char warning_lamp_status = 0;
char nrcs_data = 0;
char fault_counter = 0;
char current_speed = 0;
char speed_cmd_status = 0;
char range_stat = 0;
char byte1 = 0;
char byte2 = 0;
char byte3 = 0;
...
void cc_led_change(void)
{
        ...
        {
            cc_switch_stat = !cc_switch_stat;
            _delay((unsigned long)((200)*(20000000/4000.0)));
        }
        if(PORTBbits.RB1 == 0 && cc_switch_stat == 1)
        {
            cc_switch_stat = !cc_switch_stat;
             _delay((unsigned long)((200)*(20000000/4000.0)));
        }

        PORTCbits.RC0 = cc_switch_stat;
}
...
void change_err_stat_nLED(void)
{
 ...
 if(cc_switch_stat ==1 && fault_counter ==1)
        {
            cc_nrcs_stat = 1;
        ...
        warning_lamp_status = cc_nrcs_stat;
}
...
void nrcs_data_extract(void)
{
    ...
    nrcs_data = data_byte[2];
    nrcs_data = nrcs_data & 0x0f;


    current_speed = data_byte[3];
    current_speed = current_speed & 0x03;
...
void find_fault_data(void)
{
    if (nrcs_data == 0x00 || nrcs_data == 0x0f)
    {
        fault_counter =1;
    }
    else
...
void find_distance_from_nrcs(void)
{
   ...
   {
       if(nrcs_data == 0x01 || nrcs_data == 0x02 || nrcs_data == 0x03 || nrcs_data == 0x04)
           range_stat = 1;
       else if (nrcs_data == 0x05 || nrcs_data == 0x06 || nrcs_data == 0x07 || nrcs_data == 0x08)
           range_stat = 2;
       else if (nrcs_data == 0x09 || nrcs_data == 0x0a || nrcs_data == 0x0b || nrcs_data == 0x0c || nrcs_data == 0x0d || nrcs_data == 0x0e)
           range_stat = 3;
...
void set_byte_transmission()
{
    byte1 = speed_cmd_status;
    byte2 = (cc_switch_stat << 4) | (cc_nrcs_stat);
    byte3 = warning_lamp_status;
}
